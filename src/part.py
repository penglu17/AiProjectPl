...
def get_weather(city: str) -> str:
    ...
    返回:
        str: 包含天气信息的字符串
    """
    return f"{city} 今日天气：晴，气温22°C，湿度60%"  # 返回模拟的天气信息
...
def chat_with_agent(user_input):
    ...
    try:
        ...
        if "天气" in user_input:
            ...
            if found_cities:
                # 调用天气工具获取每个城市的天气信息
                weather_results = [get_weather(city) for city in found_cities]
                ...
                if use_ai:
                    ...
                    return response.content  # 返回 AI 整理后的回复
                else:
                    return weather_text  # 直接返回天气信息（无 AI 模式）
            else:
                # 用户提到天气但没有指定城市
                if use_ai:
                    # 使用 AI 友好地追问用户具体城市
            ...
            if use_ai:
                ...
                return "这是一个天气查询助手，请输入天气相关的问题（需要配置 AI 环境变量才能回答其他问题）"
    
    except Exception as e:
        # 捕获并处理异常
        print(f"错误: {str(e)}")       # 打印错误信息（调试用）
        return f"发生错误：{str(e)}"    # 返回错误信息给用户
